<?php
    ...
    if(isset($_GET['create']) || isset($_POST['create'])){
        ...
        //2024-03-31 00:00:00 
        $sql = "INSERT INTO `hydrant` (`id`, `nomor`, `lokasi`, `timestamp`) VALUES (NULL, '".$nomor."', '".$lokasi."', current_timestamp());";
    ...
    if(isset($_GET['update']) || isset($_POST['update'])){
        ...
        if($data){
            ...
            //2024-03-31 00:00:00 
            $sql = "UPDATE `hydrant` SET `nomor` = '".$data->nomor."', `lokasi` = '".$data->lokasi."' WHERE `hydrant`.`id` = ".$id.";";
            $result = mysqli_query($conn, $sql);
            if($result){
                http_response_code(200);
